...
<body>
	...
	<?php  
		...
		if(isset($_POST['postComment'.$post_id]))
		{
			...
			$check_empty=preg_replace('/\s+/', '', $post_body); //Remove all Spaces
			if($check_empty != "")
			{
				$date_time_now=date("Y-m-d H:i:s"); //Current Date and Time
	...
	<?php  
		...
		if($count != 0)
		{
			while($comment=mysqli_fetch_array($get_comments)) 
			{
				...
				$start_date=new DateTime($date_added); //Date and Time of Post
