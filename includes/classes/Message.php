<?php  
	class Message
	{
		...
		public function getLatestMessage($userLoggedIn,$user2)
		{
			...
			//Time ago for the Message
			$date_time_now=date("Y-m-d H:i:s");
			$start_date=new DateTime($row['date']); //Date and Time of Message
		...
		public function getCanvas()
		{
			...
			while($row = mysqli_fetch_array($query)) 
			{
				...
				if(!in_array($user_to_push, $canvas)) 
				{
				array_push($canvas, $user_to_push);
				}
			}

			foreach($canvas as $username) 
			{
		...
		public function getCanvasDropdown($data,$limit)
		{
			$page=$data['page'];
			$userLoggedIn = $this->user_obj->getUsername();
			$return_string = "";
			...
			$num_iterations=0; //Checking Number of messages
